...
function setOrdid(url,$id,$default,msg='权重') {
    Wind.css('artDialog');
    Wind.use("artDialog",'noty', function () {
        art.dialog({
            id: new Date().getTime(),
            ...
            background: "#CCCCCC",
            opacity: 0,
            content: "排序:<input type='text' id='ordid' value='"+$default+"' />",
            ok: function (e) {
                var ordid = $("#ordid").val();
                $.post(url,{"ids[]":$id,ordid:ordid},function (e) {
                    if(e.code == 1){
                        noty({
                            text: e.msg,
                            type: 'success',
