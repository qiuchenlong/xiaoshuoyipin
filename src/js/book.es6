...
(function() {
    ...
    $(($) => {
        ...
        $('.ft_li .bg_tab').on('click', function(e) {
            let $this = $(this);   // 获取到当前目标
            let index = $this.index(); // 当前索引
    ...
    $(($) => {
        //字体大小
        let _index = localStorage.getItem('index');  // 拉取记录的字体
        let index = _index==null ? 1 : parseInt(_index);  // 默认位置
        let sizes = [31, 36, 41, 46, 51];   // 字体数组
        let $minus = $('.js-size_minus');
        let $add = $('.js-size_add');
        let $comment = $('.js-comment');
        ...
        $add.on('click', function(e) {
            if (index == sizes.length - 1) {return;}   // 解决超界问题
            index += 1;
            saveIndex();
            judgeClass();
            setCss();
