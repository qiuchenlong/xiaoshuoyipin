...
if(true) {//验证成功
	...
	if($_POST['trade_status'] == 'TRADE_SUCCESS') {
			...
			//
			 ...
			 $sql = mysql_query("SELECT * FROM hb_chongzhi WHERE danhao='".$out_trade_no."' AND state=0 AND money=$total_fee");
             $mess = mysql_fetch_array($sql);       
             $uid = $mess['user_id'];

             mysql_query("UPDATE hb_user SET `money`=`money`+'".$mess[money]."' where user_id=$uid"); 
             mysql_query("UPDATE hb_chongzhi SET state=1 where id=$mess[id]");
    		}	 
