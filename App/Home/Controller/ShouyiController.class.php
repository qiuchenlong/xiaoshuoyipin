...
class ShouyiController extends Controller {
  ...
  public function index(){
    ...
    if($user==null){
      echo '<script>alert("账号已过期,请重新登录");location.href="/moneys/index.php/Home/Login/login.html"</script>';die;
    }
    $mon=$user['money'];
  	 // var_dump($mon);
    ...
    //查冻结金额
    $dong=M('touzi')->where($a)->select();
    foreach ($dong as $ke => $va) {
      $djie[]=$va['touzi'];
    }

    $dongjie=array_sum($djie);
    ...
    foreach ($info as $k => $s){
      ...
      //总资产
      $zong=$user['shouyi']+$user['money'];
      // var_dump($syzong);
      // $a=date('Y-m-d');
      // $fff=str_split($ttt,10)[0];
      //     var_dump( $fff);
      // var_dump($s);die;
      $this->assign('dang',$syzong);              //今天收益
     $this->assign('shouyi',$user['shouyi']);     //总收益
     $this->assign('dongjie',$dongjie);           //冻结金额
     $this->assign('benjin',$user['money']);     //本金
      $this->assign('zong',$zong);               //总资产
     $this->assign('tt', $tt);
  ...
  public function xiangmuxiangqing(){
    if(IS_GET){
     ...
     if(session('user_id')==NULL){
      $a="/moneys/index.php/Home/Login/zhuce.html";
    }else{
      $a="/moneys/index.php/Home/Shouyi/woyaotou.html";
    }
