...
//访问单个章节
	public function chapterreads(){ 
		...
		$data = $Model->Query("SELECT chapterid,articlename,chaptername,texts FROM jieqi_article_chapter where articleid =".$articleid." order by chapterid limit $shu,1;");
...
//小说章节
  ...
  echo '{"t":"sdhfjsdh","p":["',$varci,'"]}';
	...
	public function chapterread(){ 
        ...
        $book = $Model->Query("SELECT saleprice FROM jieqi_article_article where articleid =".$articleid.";");
		$data = $Model->Query("SELECT chapterid,articlename,chaptername,texts FROM jieqi_article_chapter where articleid =".$articleid." order by chapterid limit 0,1;");
