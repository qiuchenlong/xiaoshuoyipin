...
<section class="Hui-article-box">
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 频道管理 <span class="c-gray en">&gt;</span> 代理商结算 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav> 
			...
			<tbody id="changetr">
					...
					<tr class="text-c success">
						<td>详情</td>  
						<td>订购笔数 <?php echo ($bi); ?></td>
						<td>已打款￥<?php echo ($yi); ?> &nbsp;&nbsp;未打款￥<?php echo ($wei); ?></td>
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20"> <i ></i> </a><!-- <?php if($_SESSION["guanli"] != null): ?><a href="<?php echo U('add');?>"  class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加用户</a><?php endif; ?>  --></span><span>共有数据：<strong><?php echo ($shu); ?></strong> 条</span> </div>
			<div class="mt-0">  
				<table class="table table-border table-bordered table-bg table-hover table-sort" width="100%">
					<thead>
						...
						<?php if(is_array($arr)): foreach($arr as $key=>$v): ?><tr class="text-c"> 
							...
							<td width="30"><?php echo ($v["time"]); ?></td>
							<td width="100">
								
								<?php echo ($v["name"]); ?>
							</td>
							<td width="30"><?php echo ($v["num"]); ?></td>
							<td width="30"><?php echo ($v["money"]); ?></td>
							<td width="30"><?php echo ($v["bili"]); ?></td>
							<td width="30"><?php echo ($v["jiesuan"]); ?></td>
							<?php if($v["lei"] == 0): ?><td width="30"><font color="red"><a onclick="del('<?php echo ($v["id"]); ?>') "><button>未打款</button></a></font></td>   
							<?php else: ?><td width="30">已打款</td><?php endif; ?>
							...
							<!-- <?php if($_SESSION["guanli"] != null): ?><td >
								<a href="<?php echo U('chongzhi');?>?id=<?php echo ($v["uid"]); ?>"><button>充值看币</button></a> 
								<a title="修改" href="<?php echo U('edit');?>?id=<?php echo ($v["uid"]); ?>"  class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>  <a  href="<?php echo U('del');?>?id=<?php echo ($v["uid"]); ?>" class="ml-5" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td><?php endif; ?> -->
...
<script type="text/javascript"> 
 	function del(id){
	var con;
	con=confirm("确认已给代理商打款?"); //在页面上弹出对话框
	if(con==true){
		location.href="<?php echo U('jiesuan');?>?fuid="+id+"";
	}
}  
</script>
<!--/请在上方写此页面业务相关的脚本-->
