...
<section class="Hui-article-box">
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 频道管理 <span class="c-gray en">&gt;</span> 代理商列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav> 
	
	<div class="Hui-article">
		<article class="cl pd-20"> 
			<div class="text-c"> 
		<form action="<?php echo U('index');?>" method="post">
		<input type="text" class="input-text" style="width:250px" placeholder="输入代理商名/手机"  name="id">
		<button type="submit" class="btn btn-success" id="" name=""><i class="Hui-iconfont">&#xe665;</i> 搜索代理商</button>
		</form>
	</div> 
	

	
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <i ></i> </a><?php if($_SESSION["guanli"] != null): ?><a onclick="picture_add('添加代理商','<?php echo U('add');?>')"   class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加代理商</a><?php endif; ?> </span><span class="r">共有数据：<strong><?php echo ($shu); ?></strong> 条</span> </div>
				...
				<table class="table table-border table-bordered table-bg table-hover table-sort" width="100%">
					<thead>
						...
						<?php if(is_array($arr)): foreach($arr as $key=>$v): ?><tr class="text-c"> 
							...
							<td width="30"><?php echo ($v["email"]); ?></td>   
						
							<td width="30"><?php echo ($v["yongjin"]); ?>%</td>
							<?php if($v["jiesuan"] == 0): ?><td width="30">日结</td>
							<?php elseif($v["jiesuan"] == 1): ?><td width="30">周结</td>
							<?php elseif($v["jiesuan"] == 2): ?><td width="30">月结</td><?php endif; ?>

							<?php if($v["zhilei"] == 0): ?><td width="30">支付宝</td>
							<?php elseif($v["zhilei"] == 1): ?><td width="30">微信</td>
							<?php elseif($v["zhilei"] == 2): ?><td width="30">银行卡</td><?php endif; ?>

							
							<!-- <td width="30"><?php echo ($v["zhilei"]); ?></td> -->
							<td width="30"><?php echo ($v["zhihao"]); ?></td>
							<td width="30"><?php echo ($v["zhiname"]); ?></td>
							<?php if($v["lei"] == 1): ?><td width="30">否</td>
							<?php else: ?><td width="30"><font color="red" >是</font></td><?php endif; ?>
							
							
							
							<?php if($_SESSION["guanli"] != null): ?><td width="50">
								
								<a title="修改" onclick="picture_add('修改','<?php echo U('edit');?>?id=<?php echo ($v["uid"]); ?>')"  class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;<a  onclick="del('<?php echo ($v["uid"]); ?>') " class="ml-5" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td><?php endif; ?>
