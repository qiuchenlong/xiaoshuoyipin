...
<section class="Hui-article-box">
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 频道管理 <span class="c-gray en">&gt;</span> 女频列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav> 
	
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <i ></i> </a><?php if($_SESSION["guanli"] != null): ?><a onclick="picture_add('添加一级分类','<?php echo U('add');?>?id=1')"  class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加一级分类</a> </span> <a onclick="picture_add('添加二级分类','<?php echo U('sjadd');?>?id=1')"  class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加二级分类</a><?php endif; ?> </span> <span class="r">共有数据：<strong><?php echo ($shu); ?></strong> 条</span> </div>
				...
				<table class="table table-border table-bordered table-bg table-hover table-sort" width="100%">
					<thead>
						<tr class="text-c"> 
							...
							<?php if($_SESSION["guanli"] != null): ?><th width="50">操作</th><?php endif; ?>
							 
						</tr> 
						<?php if(is_array($arr)): foreach($arr as $key=>$v): ?><tr class="text-c"> 
							<!-- <td width="100"><?php echo ($v["sortid"]); ?></td> -->
							
							<td width="100"><?php echo ($v["shortname"]); ?></td>
							<td width="100"><img src="/Public/<?php echo ($v["imgurl"]); ?>"  width="50"></td>
							<td width="200">
								<?php  foreach($info as $j => $s){ if($s['layer']==$v['sortid']){ echo $s['shortname'].' || ' ; } } ?> <?php if($_SESSION["guanli"] != null): ?><a title="编辑二级分类"href="<?php echo U('erji');?>?id=<?php echo ($v["sortid"]); ?>&type=1"  class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a><?php endif; ?>
							</td>
							<td width="100"><?php echo ($v["description"]); ?></td>
							...
							<?php if($_SESSION["guanli"] != null): ?><td width="100"><a title="编辑一级分类" onclick="picture_add('编辑一级分类','<?php echo U('edit');?>?id=<?php echo ($v["sortid"]); ?>&type=1')"  class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> &nbsp; &nbsp; &nbsp; <a onclick="del('<?php echo ($v["sortid"]); ?>')" class="ml-5"><i class="Hui-iconfont">&#xe6e2;</i></a></td><?php endif; ?>
...
<script type="text/javascript"> 
	...
	con=confirm("确定删除吗?"); //在页面上弹出对话框
	if(con==true){
		location.href="<?php echo U('del');?>?id="+id+"&type=1";
	}
}
