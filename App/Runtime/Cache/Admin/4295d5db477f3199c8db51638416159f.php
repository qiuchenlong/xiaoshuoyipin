...
<section class="Hui-article-box">
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 频道管理 <span class="c-gray en">&gt;</span> 用户列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav> 
	
	
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <i ></i> </a><?php if($_SESSION["guanli"] != null): ?><a onclick="picture_add('添加用户','<?php echo U('add');?>')"  class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加用户</a> <a onclick="picture_add('发送信息','<?php echo U('xiaoxi');?>?id=<?php echo ($v["uid"]); ?>')" class="btn btn-success radius ">发送信息</a><?php endif; ?> </span><span class="r">共有数据：<strong><?php echo ($shu); ?></strong> 条</span> </div>
				...
				<table class="table table-border table-bordered table-bg table-hover table-sort" width="100%">
					<thead>
						<tr class="text-c"> 
							...
							<?php if($_SESSION["guanli"] != null): ?><th width="20">操作</th><?php endif; ?>
						...
						<?php if(is_array($arr)): foreach($arr as $key=>$v): ?><tr class="text-c"> 
							...
							<td width="30"><?php echo ($v["score"]); ?></td>

							
							<td width="30"><?php echo ($v["egold"]); ?></td>
							<td width="30"><?php echo ($v["daijin"]); ?></td>
							<!-- <td width="30"><a onclick="picture_add('发送信息','<?php echo U('xiaoxi');?>?id=<?php echo ($v["uid"]); ?>')" class="btn btn-success radius ">发送</a></td> -->
							
							
							
							
							
							<?php if($_SESSION["guanli"] != null): ?><td >
								<a onclick="picture_add('修改','<?php echo U('chongzhi');?>?id=<?php echo ($v["uid"]); ?>')"><button>充值看币</button>
								<a onclick="picture_add('修改','<?php echo U('daijinquan');?>?id=<?php echo ($v["uid"]); ?>')"><button>充值代金券</button>	
								</a>&nbsp; &nbsp; 
