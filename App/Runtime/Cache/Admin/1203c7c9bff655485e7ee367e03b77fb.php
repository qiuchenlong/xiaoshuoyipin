...
<title>阅读广告管理</title>  
<style>
        td p{max-height:300px;overflow:hidden !important;}
        .Hui-article-box{overflow: scroll;}
    </style>
</head>  
<body>
<!--_header 作为公共模版分离出去-->
...
<section class="Hui-article-box">
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 阅读广告管理 <span class="c-gray en">&gt;</span> 阅读广告列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav> 
	
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <i ></i> </a>
		<?php if($_SESSION["guanli"] != null): ?><a onClick="picture_add('新增轮播图','<?php echo U('addbookad');?>')" href="javascript:;" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加阅读广告</a><?php endif; ?> </span> <span class="r">共有数据：<strong><?php echo ($shu); ?></strong> 条</span> </div>
				...
				<table class="table table-border table-bordered table-bg table-hover table-sort" width="100%">
					<thead>
						...
						<?php if(is_array($arr)): foreach($arr as $key=>$v): ?><tr class="text-c"> 
							...
							<?php elseif($v["path"] == 3): ?>出版轮播
							<?php elseif($v["path"] == 4): ?>免费轮播<?php endif; ?>
							</td>
							<td width="100"><?php echo ($v["title"]); ?></td>
							<td width="100" style="text-overflow: ellipsis;"><?php echo ($v["content"]); ?></td>
							<td width="100"><?php echo ($v["weburl"]); ?></td>
							<td width="80"><a title="编辑" onClick="picture_edit('修改轮播图','<?php echo U('edit');?>?id=<?php echo ($v["id"]); ?>')" href="javascript:;"class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  <a  href="<?php echo U('del');?>?id=<?php echo ($v["id"]); ?>" class="ml-5"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
