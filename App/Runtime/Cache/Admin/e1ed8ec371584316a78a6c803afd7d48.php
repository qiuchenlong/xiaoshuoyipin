...
<section class="Hui-article-box">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <i ></i> </a><?php if($_SESSION["guanli"] != null): ?><a onclick="picture_add('添加一级分类','<?php echo U('add');?>?id=2')"  class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加一级分类</a> </span> <a onclick="picture_add('添加二级分类','<?php echo U('sjadd');?>?id=2')"  class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加二级分类</a><?php endif; ?> </span> <span class="r">共有数据：<strong><?php echo ($shu); ?></strong> 条</span> </div>
				...
				<table class="table table-border table-bordered table-bg table-hover table-sort" width="100%">
					<thead>
						...
						<?php if(is_array($arr)): foreach($arr as $key=>$v): ?><tr class="text-c"> 
							...
							<td width="100">
								<?php  foreach($info as $j => $s){ if($s['layer']==$v['sortid']){ echo $s['shortname'].' || ' ; } } if($_SESSION["guanli"] != null): ?><a title="编辑二级分类"href="<?php echo U('erji');?>?id=<?php echo ($v["sortid"]); ?>&type=2"  class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a><?php endif; ?>
							</td>
							...
							<?php if($_SESSION["guanli"] != null): ?><td ><a title="编辑一级分类" onclick="picture_add('编辑一级分类','<?php echo U('edit');?>?id=<?php echo ($v["sortid"]); ?>&type=2')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> &nbsp; &nbsp; &nbsp; <a  onclick="del('<?php echo ($v["sortid"]); ?>')" class="ml-5"><i class="Hui-iconfont">&#xe6e2;</i></a></td><?php endif; ?>
...
<script type="text/javascript"> 
 function del(id){
	...
	con=confirm("确定删除吗?"); //在页面上弹出对话框
	if(con==true){
		location.href="<?php echo U('del');?>?id="+id+"&type=2";
	}
}
